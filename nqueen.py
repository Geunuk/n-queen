...
class State():
    def __init__(self, N, coord, step=0):
        self.N = N
        self.coord = coord
        self.value = State.count_attack_total(self.coord, self.N)
        self.step = step
        self.summary = None
    ...
    @staticmethod
    def count_attack_total(coord, N):
        result = 0
        for n in range(N-1):
            result += State.count_attack_ranged(coord, n, n+1, N-1, N)
        return result
    ...
    @staticmethod
    def count_attack_solo(x, n, N):
        return State.count_attack_ranged(x, n, 0, N-1, N)
...
if __name__ == "__main__":
    ...
    s.print_board()
    print(s.attack_cnt)
    #print(State.count_attack_solo(s.coord, 0, N))
    #print(State.count_attack_solo(s.coord, 1, N))
    #print(State.count_attack_solo(s.coord, 2, N))
    print(State.count_attack_solo(s.coord, 3, N))
    """
    print_board([2,1,3,0])
