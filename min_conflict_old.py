...
def choose_variable(s):
    ...
    if choose_variable.loop_cnt != 0:
        for var in range(s.N):
            if State.count_attack_solo(s.coord, var, s.N) == 0:
...
def choose_value(s, var):
    min_val_list = []
    min_coord = s.coord[:]
    min_attack_cnt = State.count_attack_solo(min_coord, var, s.N)
    ...
    for val in values:
        tmp_coord = s.coord[:]
        tmp_coord[var] = val
        tmp_attack_cnt = State.count_attack_solo(tmp_coord, var, s.N)

        if tmp_attack_cnt == min_attack_cnt:
            min_val_list.append(val)
        elif tmp_attack_cnt < min_attack_cnt:
            min_attack_cnt = tmp_attack_cnt
            min_val_list = [val]
